...
## Agreement renewal — pipeline stage vs urgency

**Files:** `src/services/tenants/renewal-pipeline-read-model.ts` (`resolveStage`), `src/services/tenants/renewal-decision-service.ts` (`evaluateAgreement`).

- **Stage and urgency are independent axes.** `stage` is *where the renewal is* (`NEEDS_OFFER` → `DRAFT` → `INVITED` → `NEGOTIATING` → `AWAITING_PAYMENT`/`READY_FOR_SIGNATURE` → `RENEWED`, plus `DECLINED`, `OFFER_EXPIRED`, `RENEWAL_DRAFTED`, `MOVE_OUT`). `urgency` is *why it is pressing* — lapsed contract, days overdue, past grace period, overdue rent, move-out in flight, offer response window. The renewal queue's older `decision_state` fused the two (`EXPIRED_AND_RENT_OVERDUE`), which meant no offer could ever suppress the "Expired" label — see [[Bugs]].
- **Stage precedence, first match wins:** move-out in flight → predecessor already `RENEWED` → the current offer's own status (accepted-with-top-up vs accepted-clean, declined, expired/lapsed, sent-and-discussed vs sent, draft) → a successor with no accepted offer (`RENEWAL_DRAFTED`, the manual `createRenewalDraft` path) → `NEEDS_OFFER`.
- **The "current offer" is the newest non-`SUPERSEDED` one.** A superseded offer has been replaced by definition (`reviseOffer` writes it), so it must never be what a row reports as the state of play.
- **A lapsed-but-unswept offer stages as `OFFER_EXPIRED`.** Consistent with `resendOffer` and `acceptOffer` — see the resending section below.
- **Action availability is resolved server-side.** `can.{create_offer,send_offer,resend_offer,revise_offer}` are computed against the same rules the mutating services enforce, so the UI cannot offer a button whose endpoint will reject it. `create_offer` in particular is false whenever a `DRAFT`/`SENT` offer exists — including one past its window, which still blocks `generateOffer` because it is still "active" — so Resend is the only way out of a lapsed offer.
- **The pipeline is a union of two sources, keyed by agreement.** Neither alone is complete: the expiry queue only holds `EXPIRING_SOON`/`AGREEMENT_EXPIRED` agreements (so an accepted renewal drops out of it once the predecessor becomes `RENEWED`), while offers can exist against an agreement still `SIGNED`. See [[Decisions]] ADR-029.

## Agreement renewal — resending a lapsed offer

**Files:** `src/services/tenants/renewal-offer-service.ts` (`resendOffer`, `expireStaleOffers`, `reviseOffer`), `app/api/agreements/renewal-offers/[id]/resend/route.ts`.

- **A resend reuses the offer row; it does not create a second offer.** Terms are unchanged, so a cloned row would duplicate offer history without recording a new fact. The row's `status` returns to `SENT` with a new `sent_at` and `offer_expires_at`; the lapse-and-resend sequence is recorded on the timeline (`OFFER_EXPIRED` → `OFFER_SENT` with `metadata.resend = true`, carrying `previousStatus`/`previousExpiresAt`/`newExpiresAt`). Contrast `reviseOffer`, which *does* create a new row, because it changes the terms.
- **An offer past its window is resendable whether or not the sweep has run.** `expireStaleOffers` only runs from `AgreementLifecycleService.processDailyLifecycle`, so an offer can be past `offer_expires_at` while still marked `DRAFT`/`SENT`. `acceptOffer` already rejects those as expired, so `resendOffer` accepts them on the same footing as a row already flipped to `EXPIRED`. An offer still inside its window cannot be resent (`BAD_REQUEST`).
- **Resend re-derives the deposit delta rather than resending a snapshot.** `deposit_held`, `additional_deposit_required` and `deposit_refund_eligible` are recomputed from the tenant's current ledger, because those figures drive both the `AWAITING_PAYMENT` pipeline stage and the top-up obligation `acceptOffer` raises — the tenant's deposit position can have moved between the two sends.
- **One live offer per agreement still holds.** Resend refuses (`CONFLICT`) if the agreement already has a successor (`renewed_to_agreement_id`) or if a *different* `DRAFT`/`SENT` offer is still inside its window; and refuses (`BAD_REQUEST`) if the predecessor agreement has left `CURRENT_AGREEMENT_STATUSES`. Same rule `generateOffer` enforces.
- **New window defaults to 15 days**, matching offer creation; an explicit `offer_expires_at` is honoured but must be in the future.
- **`DECLINED` and `EXPIRED` offers are revisable.** Revising one is how an owner counter-offers after a decline, or re-offers on new terms after a lapse. See [[Bugs]] — the guard used to permit only `DRAFT`/`SENT`, which made the owner UI's long-standing Revise button dead on declined offers.

## Agreement renewal — successor contract terms

**Files:** `src/services/tenants/renewal-offer-service.ts` (`generateOffer`, `generateBulkOffers`, `acceptOffer`, `getAgreementContract`), `src/services/tenants/agreement-renewal-service.ts` (`resolveContractSnapshot`), `src/services/tenants/agreement-lifecycle-completeness.ts`.

- **A renewal inherits every contract term the owner didn't explicitly propose.** `contract_payment_frequency` and `contract_maintenance_type` (and `contract_maintenance`) come from the predecessor agreement — falling back to its `content_snapshot` — whenever the renewal offer doesn't carry a new value. A renewal changes what the owner proposed and nothing else; it never silently resets billing frequency or maintenance type to NULL. Applies at both offer generation and offer acceptance. See [[Decisions]] ADR-028.
- **A successor agreement must be lifecycle-complete the moment it is written.** All three paths that create a successor (`acceptOffer`, `createRenewalDraft`, cron activation) validate `REQUIRED_AGREEMENT_LIFECYCLE_FIELDS` before/at creation. `acceptOffer` refuses the acceptance with a `CONFLICT:` error naming the missing fields rather than persisting a successor that `checkLifecycleComplete` would later reject at signing time. Introduced 2026-07-31 after an incomplete successor produced an unactionable 409 on the tenant's sign step — see [[Bugs]].
- **Data repair for rows written before that rule:** `migrations/062_backfill_renewal_successor_contract_terms.sql`, scoped to **DRAFT** successors only. A signed/active agreement's contract terms are never rewritten by a backfill.

## Correction Cases — Payment corrections (Reverse / Transfer / Edit Reference)

**Files:** `backend-next/src/services/recovery/recovery-service.ts`, `correction-registry.ts`; handlers in `backend-next/src/services/payments/corrections/` — `payment-reversal-handler.ts`, `payment-transfer-handler.ts`, `reference-edit-handler.ts`, plus the shared `payment-correction-shared.ts::reverseObligationPayment`. Schema: see [[Database]] `correction_cases`/`correction_case_events`.
