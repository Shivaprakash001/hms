...
`/api/agreements/history`, `/api/agreements/renewal-pipeline` (GET — **unified owner renewal pipeline**, `hostelId` required, optional `stage`: one row per agreement carrying a single lifecycle `stage` (`NEEDS_OFFER`/`DRAFT`/`INVITED`/`NEGOTIATING`/`AWAITING_PAYMENT`/`READY_FOR_SIGNATURE`/`RENEWAL_DRAFTED`/`RENEWED`/`DECLINED`/`OFFER_EXPIRED`/`MOVE_OUT`), a separate `urgency` block, and server-resolved `can.{create_offer,send_offer,resend_offer,revise_offer}` action flags. Composes `getOwnerRenewalQueue` + `listOffers` as a **union** keyed by agreement — `renewal-pipeline-read-model.ts`, see [[Decisions]] ADR-029, [[Business-Rules]]. Backs the merged owner screen; counts are always whole-pipeline, never scoped to the selected stage), `/api/agreements/renewals` (owner queue — still live, still the source of the expiry/rent/move-out half of the pipeline read model above), `/api/agreements/renewals/[agreementId]` (GET — Individual Renewal Workspace read model: current agreement, successor draft, full offer history, `RenewalTimelineEvent` timeline, `financialReadModelService` summary, `identificationDocument`s, and activation readiness once a successor exists — see [[Frontend]], [[Decisions]]), `/api/agreements/[id]/lifecycle-recovery`, `/api/agreements/[id]/renewal-draft`, `/api/agreements/[id]/renewal-offer`, `/api/agreements/[id]/sign-renewal`, `/api/agreements/renewal-offers` (GET list / POST bulk-generate — FLAT/PERCENTAGE/ROOM_CATEGORY/FLOOR_WISE/ROOM_WISE strategy, see [[Business-Rules]]), `/api/agreements/renewal-offers/[id]` (PATCH revise/supersede — accepts `DRAFT`/`SENT`/`DECLINED`/`EXPIRED` predecessors, so revising is how an owner counter-offers after a decline or re-offers on new terms after a lapse), `/api/agreements/renewal-offers/[id]/send`, `/api/agreements/renewal-offers/[id]/resend` (POST — re-send an expired offer on the *same* terms with a fresh response window; optional body `{ offer_expires_at }`, default +15 days. 400 if the offer is not expired, 409 if the agreement was already renewed or a newer live offer exists — see [[Business-Rules]], [[Features]]),  `/api/agreements/renewal-audiences`, `/api/agreements/r4-readiness`, `/api/agreements/lifecycle-recovery` (+`/completion`, `/export`). Tenant side: `/api/tenant/agreement-renewal`, `/api/tenant/renewal-offer` (+`/[id]/accept|decline|discuss`), `/api/tenants/me/renewal-signature` (POST — session-authenticated ImageKit signature upload for the `sign-renewal` step above, mirrors `tenants/activate/signature` but resolves the tenant from session instead of an activation token; consumed by `/tenant/renewal`, see [[Frontend]], [[Decisions]] ADR-019), `/api/tenant/exit` (owner-processed exit, distinct from the move-out workflow below).

**Error contract — `POST /api/tenant/renewal-offer/[id]/accept`:** maps the service's message prefixes to `NOT_FOUND` → 404, `FORBIDDEN` → 403, `BAD_REQUEST` → 400, `CONFLICT` → 409, anything else → 500. The `CONFLICT` → 409 mapping was added 2026-07-31; before that both conflict cases the service can raise ("a renewal was already accepted for this agreement", and the new missing-contract-terms guard) surfaced as 500s. `POST /api/agreements/[id]/sign-renewal` returns 409 with an `AgreementRenewalSigningError` code (`AGREEMENT_LIFECYCLE_INCOMPLETE`, `PREDECESSOR_NOT_RENEWABLE`, `SECURITY_DEPOSIT_UNPAID`, `MOVE_OUT_IN_PROGRESS`, `RENEWAL_DRAFT_REQUIRED`, `INVALID_RENEWAL_CHAIN`, `SIGNATURE_REQUIRED`) — see [[Bugs]] for the `AGREEMENT_LIFECYCLE_INCOMPLETE` case that made this a guaranteed failure for every offer-accepted renewal, and [[Business-Rules]] for the rule that now prevents it upstream.
