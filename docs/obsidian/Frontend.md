...
`platforms/owner/` contains **no page components**, only `router/OwnerRoutes.tsx` and `router/OwnerProviderShell.tsx`. Real owner screens live under `app/components/views/*`: `PortfolioView`, `HostelsView`, `TenantsPortfolioView`, `TenantProfileRoute`, `BulkInvitationImportView`, `MoveOutsView`, `RenewalPipelineView`, `AlertsView`, `BillingView`, `SettingsView`, `ActivityLogsView`, `AdmissionsView`, `HostelActivityCenterView`, plus a `billing/` sub-tree of dashboard widgets (`FinancialControlCenter`, `CashflowCharts`, `CollectionPipeline`, `OverdueIntelligence`, `PaymentLedger`, `RiskZone`, `RoomPerformance`, `SmartFilters`, etc. — ~20 components).

`RenewalPipelineView` (2026-08-22, superseding `RenewalQueueView`) is the **single** owner renewal screen — the former Expiring Stays / Offers Pipeline tab pair and the standalone `RenewalWorkspaceView` detail route were merged into it, see [[Decisions]] ADR-029. It is a thin orchestrator (one `getRenewalPipeline` query + the offer mutations + stage/room filter state) composing `app/components/views/renewal/`: `RenewalPipelineTracker` (compact always-visible 4-up grid, now wired to pipeline stage counts), `RenewalPipelineList` (one row per agreement showing lifecycle `stage` as the headline badge and urgency — lapsed contract, overdue rent, response window, deposit top-up — as separate chips; groups into "Needs you" vs "Waiting on tenant"; checkbox multi-select limited to rows the server marks `can.create_offer`, feeding a scoped `BulkCampaignSheet`), `RenewalDetailSheet` (the slide-over replacing the workspace page — lazily fetches the unchanged `GET /api/agreements/renewals/[agreementId]` on open), and the three pre-existing `ResponsiveDialog` sheets `BulkCampaignSheet`/`SingleOfferSheet`/`ReviseOfferSheet`, which were kept as-is and fed adapted row shapes from the view rather than forked. Row and sheet action buttons render strictly from the server's `can.*` flags — the old lists re-derived availability client-side, which is how a Create Offer button that 409'd and a Revise button that 400'd both shipped (see [[Bugs]]). `/agreements/renewals/:agreementId` now renders this same view with that row's sheet pre-opened, preserving old deep links. Deleted in the merge: `RenewalQueueView.tsx`, `RenewalWorkspaceView.tsx`, `RenewalQueueList.tsx`, `RenewalOffersList.tsx`.

New shared primitive: `app/components/ui/responsive-sheet.tsx` (`ResponsiveSheet*`), a sibling of `responsive-dialog.tsx` for the *detail panel* case rather than the focused-task case — a right-edge slide-over on desktop (the list keeps its scroll position behind it) and the same swipe-to-dismiss bottom `Drawer` on mobile, chosen by `useIsMobile`. Exported from `@shared/ui`.

`AgreementLifecycleRecoveryView.tsx` (the other page under the "Agreements" bottom-nav tab, a one-time legacy-data-migration checklist) was likewise made mobile-first: the status banner doubles as an expand/collapse control (defaults collapsed once fully recovered), the worklist is card rows instead of a `<table>`, and its recovery-form modal moved onto `ResponsiveDialog`. `app/components/BottomNav.tsx`'s "Agreements" tab now points at `/agreements/renewals` (was `/agreements/lifecycle-recovery`) and highlights active for any `/agreements/*` path; `app/components/OwnerQuickActions.tsx`'s global FAB now hides on `/agreements/*` (those pages have their own primary actions). See [[Changelog]], [[Bugs]].
