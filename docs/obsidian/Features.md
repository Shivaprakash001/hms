...
| Correct Payment (Reverse only) | Surfaced within `TenantProfilePage.tsx` → Financial Activity feed (per-payment "Correct Payment" button) | `CorrectPaymentModal` (`frontend-v2/src/app/components/modals/`) → `features/recovery/api` (`recoveryService`) → `/api/recovery/cases*` (Business Recovery Platform) — see below |
...
### Correct Payment (Reverse only)
- **Status:** shipped (Reverse only — first real UI consumer of the Business Recovery Platform's `/api/recovery/cases/*` routes)
- **Owner-facing?** yes · **Tenant-facing?** no
- **Key files:** `frontend-v2/src/features/recovery/api/index.ts` (`recoveryService.createCase/validate/execute/getById`), `frontend-v2/src/app/components/modals/CorrectPaymentModal.tsx` (self-contained reason → preview → confirm modal), `frontend-v2/src/features/tenants/components/financial/FinancialActivityCard.tsx` (`onCorrectPayment` prop, "Correct Payment" button next to "View Receipt"), `FinancialActivity.tsx` (threads the prop), `frontend-v2/src/features/tenants/components/profile/TenantProfilePage.tsx` (`correctingPaymentId` state, renders the modal, invalidates `queryKeys.tenants.{obligations,financialTimeline,advance,full,overview}` on close)
- **Depends on:** [[APIs]] (Recovery / Corrections), [[Business-Rules]] (Correction Cases — Payment corrections)
- **Notes:** Wired directly into the Tenant Details page's Financial Activity feed per an explicit product decision to **not** build a separate Recovery Center dashboard in this phase (that list/detail view is deferred to Phase 3). This bypasses the Owner Action Registry entirely — it does not register as a `CORRECTION`-category owner action; the "Correct Payment" button is a conditional render (`receiptPaymentId && onCorrectPayment && canCorrectPayment`) alongside the existing "View Receipt" button, not sourced from `ownerActionsService`. The third condition (`canCorrectPayment = payments.length <= 1`) hides the button for grouped/multi-payment settlement entries — see [[Bugs]] — because reversing one payment in a group would only be a partial reversal of the card's total. The modal calls `createCase` (which also runs preview server-side in one round trip) on "Preview Impact", then on "Confirm Reversal" calls `validate` followed by `execute` as a single logical step — if `validate` rejects (422 `VALIDATION_REJECTED`, e.g. already reversed or payment gone), `execute` is never called and the rejection reason is shown inline. Transfer (`PAYMENT_TRANSFER`) and Edit Reference/Notes (`PAYMENT_REFERENCE_EDIT`) UIs are explicitly out of scope for this slice — the backend handlers exist and are reviewed, but have no frontend entry point yet.
