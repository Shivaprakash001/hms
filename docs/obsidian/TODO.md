...
- [ ] Whether `components/landing-v2/*` or `components/marketing/*` is the live marketing component set in `frontend-v2` — both exist with overlapping names. See [[Frontend]].
- [ ] Whether `frontend-v2/src/services/index.ts` (root barrel) still has any consumers, or is dead. See [[Frontend]].
