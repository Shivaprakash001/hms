...
| Create manual renewal draft | `POST /api/agreements/{id}/renewal-draft` | 🔴 route permits `TENANT` role — **is a tenant-initiated renewal draft (without an owner-generated offer at all) an intended product flow, or is this route accidentally over-permissioned?** No frontend surface for this was found in `frontend-v2` for the tenant role; flagging rather than asserting either way. |
