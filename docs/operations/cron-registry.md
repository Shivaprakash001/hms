...
Validation notes:
- `backend-next/vercel.json` is the only Vercel config with cron definitions.
- `frontend/vercel.json` and `frontend-v2/vercel.json` do not define crons.
...
Removed from `backend-next/vercel.json`:
...
Enabled `/api/cron/daily-briefings` in `backend-next/vercel.json`.
