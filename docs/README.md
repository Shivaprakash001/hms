...
`frontend-v2/` is the canonical future UI.
`backend-next/` is the canonical API and data service.
`frontend/` is legacy implementation evidence.
`temp-ui/` is a Figma/prototype reference.

**How this works:**
1. Screens are documented from `frontend-v2/src`.
2. API behavior is documented from `backend-next/app/api` and service files.
3. Database behavior is documented from `backend-next/prisma/schema.prisma`.
