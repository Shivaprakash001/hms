 ...
 * POST /api/students/ — Enroll a new student (admin/warden only)
 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return apiError(error.message || "Failed to fetch students");
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const student = await studentService.createStudent(body, session.sub);
    return apiResponse(student, 201);
  } catch (error: any) {
    return apiError(error.message || "Failed to create student enrollment");
  }
}
