...
export async function streamExpensesXlsx(req: ExpenseExportRequest): Promise<ReadableStream<Uint8Array>> {
  ...
  addRow(["Generated at", new Date(metadata.generatedAt).toLocaleString("en-IN")]);
...
function reportDateLabel(filters: ExpenseFilters, start: Date, end: Date): string {
  ...
  const sameMonth = start.getFullYear() === inclusiveEnd.getFullYear() && start.getMonth() === inclusiveEnd.getMonth();
  if (sameMonth) return start.toLocaleDateString("en-IN", { month: "long", year: "numeric" });
  return "Custom Range";
}
...
export async function generateExpensesPdf(req: ExpenseExportRequest): Promise<Uint8Array> {
  ...
  text(`Generated at: ${new Date(metadata.generatedAt).toLocaleString("en-IN")}`, { size: 9.5 });
  ...
  const generatedOn = new Date(metadata.generatedAt).toLocaleDateString("en-IN");
