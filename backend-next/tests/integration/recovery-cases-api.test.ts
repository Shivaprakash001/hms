...
// `getSession` (backend-next/lib/auth-edge.ts) reads plain headers set by
// middleware in production (x-user-id / x-user-role / x-owner-id / x-tenant-id).
// Route-level integration tests that call handlers directly (bypassing
// middleware) set those headers on the NextRequest themselves — see
