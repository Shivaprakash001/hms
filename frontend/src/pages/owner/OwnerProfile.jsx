...
import { User, Building2, Settings, Save, Loader2, ChevronRight, ChevronDown, Receipt, Bell, CreditCard, Shield, Zap, Globe, IndianRupee, Calendar, Clock, FileText, ToggleLeft, ToggleRight, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
// ─── Preferences Module Definitions ───────────────────────────
const prefModules = [
...
export default function OwnerProfile() {
    ...
    // Expanded preferences state (7 modules)
    const [preferences, setPreferences] = useState({
        ...
        auto_rent_day: 1,
        due_day: 5,
        late_fee_type: 'none',        // 'none' | 'flat' | 'percentage'
        late_fee_amount: 200,
        ...
        auto_deactivate_days: 0,    // 0 = disabled
        // Receipts
        receipt_prefix: 'HMS',
        ...
        data_retention_months: 0,     // 0 = forever
        // System
    ...
    // Track which preference module is open
    const [openModule, setOpenModule] = useState(null);
    // Track which modules have unsaved changes
        ...
        const load = async () => {
            ...
            try {
                ...
                setPreferences(prev => ({
                    ...
                    rent_cycle: prefs.rent_cycle || prev.rent_cycle,
                    receipt_prefix: prefs.receipt_prefix || prev.receipt_prefix,
                    timezone: prefs.timezone || prev.timezone,
                    auto_rent_day: prefs.auto_rent_day || prev.auto_rent_day,
                    phonepe_merchant_id: prefs.phonepe_merchant_id || '',
                    upi_id: hostel.upi_id || '',
                    // Load any new fields the backend returns
    ...
    const updatePref = (key, value) => {
        ...
        // Mark the current open module as dirty
        if (openModule) {
            setDirtyModules(prev => new Set(prev).add(openModule));
        }
    };
    ...
    const savePreferences = async (moduleKey) => {
        setSaving(true); setError('');
        try {
            // Save UPI to hostel if changed
            ...
            const preferencePayload = {
                ...
                // Extended fields — backend will accept what it knows
                due_day: preferences.due_day,
                late_fee_type: preferences.late_fee_type,
                late_fee_amount: preferences.late_fee_amount,
                late_fee_percentage: preferences.late_fee_percentage,
                late_fee_after_days: preferences.late_fee_after_days,
                max_late_fee: preferences.max_late_fee,
                auto_generate_rent: preferences.auto_generate_rent,
                auto_apply_late_fees: preferences.auto_apply_late_fees,
                auto_send_reminders: preferences.auto_send_reminders,
                auto_deactivate_days: preferences.auto_deactivate_days,
                auto_email_receipt: preferences.auto_email_receipt,
                receipt_format: preferences.receipt_format,
                receipt_footer: preferences.receipt_footer,
                require_doc_approval: preferences.require_doc_approval,
                allow_tenant_edits: preferences.allow_tenant_edits,
                data_retention_months: preferences.data_retention_months,
                date_format: preferences.date_format,
                language: preferences.language,
                // Notification prefs
                reminder_email: preferences.reminder_email,
                reminder_in_app: preferences.reminder_in_app,
                reminder_whatsapp: preferences.reminder_whatsapp,
                reminder_day_1: preferences.reminder_day_1,
                reminder_day_5: preferences.reminder_day_5,
                reminder_day_10: preferences.reminder_day_10,
                late_fee_notification: preferences.late_fee_notification,
                owner_daily_summary: preferences.owner_daily_summary,
            };

            await ownerService.updatePreferences(preferencePayload);

            // Clear dirty flag for this module
            setDirtyModules(prev => { const n = new Set(prev); n.delete(moduleKey); return n; });
            showTempSuccess(`${moduleKey.charAt(0).toUpperCase() + moduleKey.slice(1)} settings saved`);
        } catch (e) {
            const detail = e?.response?.data?.detail;
    ...
    return (
        <div className="space-y-4 pb-8 max-w-2xl mx-auto">
            ...
            {/* ─── Preferences Tab (7 Accordion Modules) ─── */}
            {activeTab === 'preferences' && (
                <div className="space-y-2">
                    {prefModules.map((mod) => {
                        const Icon = mod.icon;
                        const isOpen = openModule === mod.key;
                        const isDirty = dirtyModules.has(mod.key);
                        return (
                                ...
                                {isOpen && (
                                    <div className="px-4 pb-4 pt-1 border-t border-slate-100 space-y-4">
                                        ...
                                        <button
                                            type="button"
                                            disabled={saving}
                                            onClick={() => savePreferences(mod.key)}
                                            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 active:bg-indigo-800 transition-colors disabled:opacity-60"
                                        >
                                            {saving ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}
                                            {saving ? 'Saving...' : `Save ${mod.label}`}
                                        </button>
                                    </div>
                                )}
                            </div>
        ...
        <div className="space-y-3">
            ...
            {prefs.late_fee_type !== 'none' && (
                <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 text-sm">
                    <p className="text-indigo-700 font-medium mb-1.5 flex items-center gap-1.5">
                        <Calendar size={14} /> Billing Preview
                    </p>
                    <div className="text-indigo-600 text-xs space-y-0.5">
                        <p>Day 1–{prefs.late_fee_after_days} → ₹8,000 (rent only)</p>
                        <p>Day {prefs.late_fee_after_days + 1}+ → ₹8,000 + ₹{prefs.late_fee_type === 'flat' ? prefs.late_fee_amount : `${prefs.late_fee_percentage}%`} late fee</p>
                        {prefs.max_late_fee > 0 && <p>Cap → ₹{prefs.max_late_fee} maximum</p>}
                    </div>
                </div>
            )}
        </div>
...
function PaymentsModule({ prefs, updatePref }) {
    return (
        <div className="space-y-3">
            <Field label="Hostel UPI ID" value={prefs.upi_id || ''} onChange={(v) => updatePref('upi_id', v)}
...
function NotificationsModule({ prefs, updatePref }) {
    return (
        <div className="space-y-3">
            ...
            <ToggleField label="WhatsApp" desc="Send via WhatsApp (coming soon)" value={prefs.reminder_whatsapp}
                onChange={(v) => updatePref('reminder_whatsapp', v)} disabled />
        ...
        <div className="space-y-3">
            ...
            <SelectField label="Auto-Deactivate After" value={String(prefs.auto_deactivate_days)}
                options={[
                    ...
                    { value: '30', label: '30 days unpaid' },
                    { value: '60', label: '60 days unpaid' },
                    { value: '90', label: '90 days unpaid' },
                ]}
    ...
    return (
        <div className="space-y-3">
            ...
            <div>
                ...
                <textarea value={prefs.receipt_footer}
                    onChange={(e) => updatePref('receipt_footer', e.target.value)}
...
function ToggleField({ label, desc, value, onChange, disabled = false }) {
    return (
        ...
        >
            ...
            {value
                ? <ToggleRight size={24} className="text-indigo-600 flex-shrink-0" />
                : <ToggleLeft size={24} className="text-slate-300 flex-shrink-0" />
            }
        </button>
    );
}
