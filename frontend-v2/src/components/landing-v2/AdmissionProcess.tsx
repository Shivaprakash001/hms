...
export function AdmissionProcess({ steps = fallbackLandingContent.admissionSteps }: { steps?: AdmissionStepContent[] }) {
  return (
        ...
        <StaggerReveal>
          <div className="hidden md:flex justify-between items-start relative max-w-5xl mx-auto">
            {steps.map((step, index) => {
              const Icon = getLandingIcon(step.icon, 'phone');
              return (
                <StaggerItem key={index}>
                  <div className="flex flex-col items-center relative" style={{ width: '180px' }}>
                    ...
                    <div className={`w-12 h-12 rounded-full ${step.isFinal ? 'bg-[#1B2D5B]' : 'bg-[#F07B1D]'} text-white flex items-center justify-center font-bold text-xl mb-4 relative z-10`}>
                      {step.number}
                    </div>

                    <div className={`w-14 h-14 rounded-full ${step.isFinal ? 'bg-[#1B2D5B]/10' : 'bg-[#F07B1D]/10'} flex items-center justify-center mb-3`}>
                      <Icon className={`w-7 h-7 ${step.isFinal ? 'text-[#1B2D5B]' : 'text-[#F07B1D]'}`} />
                    </div>
                ...
                {steps.map((step) => {
                  ...
                  return (
                    <div key={step.number} className="relative flex items-center gap-3 rounded-xl bg-[#FFFDF5] px-3 py-3">
                      <div className={`relative z-10 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full ${step.isFinal ? 'bg-[#1B2D5B]' : 'bg-[#F07B1D]'} text-sm font-bold text-white shadow-sm`}>
                        {step.number}
                      </div>
                      <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full ${step.isFinal ? 'bg-[#1B2D5B]/10' : 'bg-[#F07B1D]/10'}`}>
                        <Icon className={`h-4 w-4 ${step.isFinal ? 'text-[#1B2D5B]' : 'text-[#F07B1D]'}`} />
                      </div>
