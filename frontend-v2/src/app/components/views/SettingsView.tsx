...
export function SettingsView() {
  ...
  const name = String(profile?.name ?? profile?.full_name ?? '');
  ...
  const handleItemClick = (label: string) => {
    if (label === 'Pricing & Rates') setShowPricingRates(true);
  };
  ...
  return (
    <div className="px-4 py-6 space-y-6">
      ...
      <button
        onClick={logout}
        className="w-full bg-destructive/10 text-destructive p-4 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition-transform"
      >
