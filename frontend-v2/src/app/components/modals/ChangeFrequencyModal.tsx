...
function newRow(): InstallmentRow {
  return { id: crypto.randomUUID(), due_date: todayIso(), amount: '', label: '' };
}
            ...
            {mode === 'RECURRING' ? (
              ...
              <div className="space-y-3">
                <label className="text-xs font-semibold text-foreground flex items-center justify-between">
                  <span>Installments <span className="text-rose-500">*</span></span>
                  <span className="text-[11px] font-normal text-muted-foreground">{rows.length} of max 24</span>
                </label>
                <div className="space-y-2.5 max-h-[280px] overflow-y-auto pr-1">
                  {rows.map((row, i) => (
                    <div key={row.id} className="p-3 rounded-2xl border border-border/80 bg-muted/20 space-y-2.5 transition-all hover:border-border">
                      <div className="flex items-center justify-between border-b border-border/40 pb-1.5">
                        ...
                        <button
                          ...
                          className="p-1 rounded-lg text-muted-foreground hover:bg-rose-500/10 hover:text-rose-600 transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-muted-foreground"
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            className="w-full rounded-xl border border-input bg-background px-2.5 py-1.5 text-xs text-foreground focus:ring-1 focus:ring-accent focus:outline-none"
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[10px] font-medium text-muted-foreground">Amount (₹)</label>
                          <input
                            type="number" min="1" step="1" placeholder="e.g. 33500"
                            value={row.amount}
                            onChange={(e) => updateRow(row.id, { amount: e.target.value })}
                            className="w-full rounded-xl border border-input bg-background px-2.5 py-1.5 text-xs text-foreground placeholder:text-muted-foreground/60 focus:ring-1 focus:ring-accent focus:outline-none"
                          />
                        </div>
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-medium text-muted-foreground">Label / Note (optional)</label>
                        <input
                          type="text"
                          placeholder="e.g. 1st Term / Admission Fee"
                          value={row.label}
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-dashed border-accent/40 text-xs font-semibold text-accent hover:bg-accent/5 transition-colors disabled:opacity-40"
                >
                  <Plus className="w-3.5 h-3.5" />
                  Add another installment
                </button>
                <div className="flex justify-between items-center text-xs bg-muted/40 p-2.5 rounded-xl border border-border/50">
                  <span className="text-muted-foreground">{rows.length} installment{rows.length === 1 ? '' : 's'}</span>
                  <span className="font-bold text-foreground">Total: {fmt(customTotal)}</span>
                </div>
                <p className="text-[11px] text-muted-foreground px-1">
