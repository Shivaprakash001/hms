...
def get_all_students(
    ...
    try:
        ...
        # Include pending invitations as INVITED records for owner tenant list visibility
        if owner_id and (status is None or str(status).upper() == "INVITED"):
            invite_res = supabase.table("invitations")\
                .select("id, email, name, phone, room_id, monthly_rent, created_at, status")\
                .eq("owner_id", owner_id)\
                .eq("status", "PENDING")\
                .order("created_at", desc=True)\
                .execute()

            invitations_raw = invite_res.data or []
            room_ids = list({inv.get("room_id") for inv in invitations_raw if inv.get("room_id")})
            rooms_by_id = {}
            if room_ids:
            ...
            for inv in invitations_raw:
                ...
                invited_row = {
                    ...
                    "profile": {
                        "name": inv.get("name") or "Invited Tenant",
                        "email": inv.get("email"),
                        "phone": inv.get("phone")
                    },
                    "current_room": room,
                    "payment_summary": {
                        ...
                        "last_paid_at": None,
                        "last_paid_amount": 0,
                        "current_obligation_id": None
                    }
                }

                if search:
                    profile = invited_row.get("profile") or {}
                    current_room = invited_row.get("current_room") or {}
                    name = (profile.get("name") or "").lower()
                    email = (profile.get("email") or "").lower()
                    phone = (profile.get("phone") or "").lower()
                    room_no = (current_room.get("room_no") or "").lower()
                    q = search.lower()
                    if q not in name and q not in email and q not in phone and q not in room_no:
                        continue

                students_data.append(invited_row)

        # Consistent latest-first ordering after mixing students + invitations
        def _sort_key(item: dict):
            return item.get("created_at") or item.get("joined_on") or ""

        students_data.sort(key=_sort_key, reverse=True)
