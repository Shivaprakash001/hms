...
| Tree | Role |
|---|---|
| `backend-next/` | **Canonical API** — Next.js 14 App Router + Prisma + Postgres (Supabase). All business logic lives here. |
| `frontend-v2/` | **Canonical UI** — Vite + React 19 SPA. Owner app, tenant portal, and public marketing site. |
...
Env vars load from `../.env` (repo root), not `backend-next/.env` — scripts use `DOTENV_CONFIG_PATH=../.env`. `.env.test` is used by the test suite (`vitest.config.ts` loads it first, then falls back to `.env`).
...
`frontend-v2` page → feature hook (`useQuery`/`useMutation`) → feature API wrapper (`src/features/*/api`) → Axios client (`src/lib/api-client.ts`) → `backend-next/app/api/*` route → service (`lib/services/` or `src/services/`) → Prisma → Postgres.

Query cache keys are centralized in `frontend-v2/src/lib/queryKeys.ts`; mutations invalidate by key, and the UI updates from cache rather than manual refetches.

### `backend-next/` internals
...
- `frontend-v2/scripts/check-architecture.mjs`: no raw `fetch()`/`axios` in `app/`, `platforms/`, `shared/ui`, `features/`, `portal/`, or `context/` — everything must go through `@lib/api-client`; `src/portal` is frozen (allowlist-based).
- `backend-next/scripts/architectural-invariants-check.ts`: e.g. operational backend code must not treat `hostelId` as optional, must not fall back to "first hostel" (`hostels[0]`), and dashboard cache invalidation must go through the one blessed helper — these exist because past bugs picked the wrong hostel silently for multi-hostel owners.
...
- Access token: JWT, 20 min expiry, sent in memory only (not persisted).
- Refresh token: hashed in Postgres (`refresh_tokens.token_hash`), delivered via secure httpOnly `SameSite=Lax` cookie (`hms_refresh_token`).
- Idle timeout 30 min, absolute owner/admin session cap 12 hours; refresh-token reuse triggers session revocation (replay detection).
- A 401 triggers an automatic refresh; refresh failure clears local storage and forces re-login.
